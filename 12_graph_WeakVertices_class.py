...
class SimpleGraph:
    ...
    def WeakVertices(self):
        ...
        for vertex in self.vertex:
            ...
            edge_index = 0 
            for edge in self.m_adjacency[self.vertex.index(vertex)]:
                if edge and self.vertex[edge_index] is not vertex:
                    vertex_to_check.append(self.vertex[edge_index])
                edge_index += 1
            
            for vertex_from in vertex_to_check:
                for vertex_to in vertex_to_check:
                    if (self.m_adjacency[self.vertex.index(vertex_from)][self.vertex.index(vertex_to)] or self.m_adjacency[self.vertex.index(vertex_to)][self.vertex.index(vertex_from)]) and vertex_to is not vertex_from:
                        strong = True
            
            if not strong:
                list_to_return.append(vertex)
